...
export default function ChatPanel() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      addChatMessage({ role: 'model', content: "Here's the updated component:" });
      setGeneratedCode(data.data);

    } catch (err) {
      const errorMsg = err instanceof Error ? err.message : 'An unknown error occurred';
      setError(errorMsg);
      addChatMessage({ role: 'model', content: `Sorry, I ran into an error: ${errorMsg}`});
    } finally {
      setAILoading(false);
    }
  };
  ...
  return (
    <div className="flex flex-col h-full bg-gray-800">
      <div className="flex-1 p-4 overflow-y-auto space-y-4">
        {chatHistory.length === 0 && (
          <div className="text-center text-gray-400 text-sm">
            Start by describing a component. For example: "Create a simple pricing card with a title, price, and a feature list."
          </div>
        )}
      ...
      <form onSubmit={handleSubmit} className="p-4 border-t border-gray-700">
        ...
        />
        <button type="submit" disabled={isLoadingAI || !prompt.trim()} className="w-full mt-2 px-4 py-2 font-bold text-white bg-blue-600 rounded-lg hover:bg-blue-700 disabled:bg-gray-500">
          {isLoadingAI ? 'Generating...' : chatHistory.length === 0 ? 'Generate' : 'Update'}
        </button>
      </form>
    </div>
  );
